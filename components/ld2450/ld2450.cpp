...
#define PERSON_PUBLISH_(name, person, val)             \
    ...
    if (name##_##person != nullptr)                    \
        name##_##person->publish_state(val);
...
#define lowbyte(val) (uint8_t)((val) & 0xff)
                        ...
                        if (response_frame_header.command_ack)
                        {
                            ...
                            case GET_REGIONS:
                                ...
                                for (int i = 0; i < 3; i++)
                                {
                                    ...
                                    if (x0 != nullptr)
                                        x0->publish_state(sensor_regions.coordinates[i].X0 / 10);
                                    if (y0 != nullptr)
                                        y0->publish_state(sensor_regions.coordinates[i].Y0 / 10);
                                    if (x1 != nullptr)
                                        x1->publish_state(sensor_regions.coordinates[i].X1 / 10);
                                    if (y1 != nullptr)
                                        y1->publish_state(sensor_regions.coordinates[i].Y1 / 10);
                                }
                                break;
#endif
                            }
        ...
        void LD2450::set_single_target(bool enable)
        {
            ...
            if (enable)
                this->set_single_target_();
            else
                this->set_multi_target_();
        ...
        void LD2450::set_region(uint8_t region)
        {
            ...
            if (x0->has_state())
                sensor_regions.coordinates[region].X0 = x0->state * 10;
            if (y0->has_state())
                sensor_regions.coordinates[region].Y0 = y0->state * 10;
            if (x1->has_state())
                sensor_regions.coordinates[region].X1 = x1->state * 10;
            if (y1->has_state())
                sensor_regions.coordinates[region].Y1 = y1->state * 10;
        }
#endif
        ...
        void LD2450::report_position(void)
        {
            ...
            bool moving_target = (received_data.person[0].speed ||
                                  received_data.person[1].speed ||
                                  received_data.person[2].speed);

            bool still_target = ((received_data.person[0].resolution && !received_data.person[0].speed) ||
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                if (received_data.person[i].resolution)
                {
                    if (exiting)
                        ...
                        presence_millis[i] = current_millis + presence_timeout * 1000;
                }
            }

            bool target = ((received_data.person[0].resolution || presence_millis[0] > current_millis) ||
